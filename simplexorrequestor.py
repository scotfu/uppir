...
class RandomXORRequestor:
  ...
  def __init__(self, mirrorinfolist, blocklist, manifestdict, privacythreshold, pollinginterval = .1):
    ...
    self.actural_mirror_list[-1]['blockbitstringlist']=[]
    ...
    for blocknum in range(len(blocklist)):
      ...
      for requestinfo in self.actural_mirror_list[:-1]:
      ...
      self.actural_mirror_list[-1]['blockbitstringlist'].append(thisbitstring)
    ...
    self.neededreturnedxorblocksdict = {}
    ...
    for blocknum in blocklist:
      ...
      self.neededreturnedxorblocksdict[blocknum] = []
    ...
    try:
      ...
      for activemirrorinfo in self.activemirrorinfolist:
        ...
        if activemirrorinfo['mirrorinfo'] == thismirrorsinfo:
          ...
          if thismirrorsinfo in [m['mirrorinfo'] for m in self.actural_mirror_list]:
            self.neededreturnedxorblocksdict[blocknumber].append(xorblockdict)
          ...
          resultingblock = _reconstruct_block(self.neededreturnedxorblocksdict[blocknumber])
...
#          print 'return',self.finishedblockdict[blocknumber]          
